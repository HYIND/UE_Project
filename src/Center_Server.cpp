...
void Center_Server::HeartBeatCheck_Process()
{
    while (!Process_stop)
    {
        for (auto map_it = HeartBeat_map.begin(); map_it != HeartBeat_map.end(); map_it++)
        {
            ...
            else if (map_it->second.count < 6 && map_it->second.count >= 0)
            {
                (map_it->second.count)++;
            }
        }
...
void Center_Server::OnLogout(const int socket_fd, const Header header, const char *content)
{
    ...
    if (userinfo)
    {
        ...
        RemoveFd(userinfo->Get_SocketFd());
...
void Center_Server::RemoveFd(int fd)
{
    try
    {
        delfd(epoll, fd);
        ...
        for (auto it = user_list.begin(); it != user_list.end(); it++)
        {
            ...
            LOGINFO("Center_Server::RemoveFd timeout, user logout : {}", (*it)->Get_UserName());
        }
    ...
    {
        perror(e.what());
        LOGINFO("Center_Server::RemoveFd , an unknown error :{}", e.what());
    }
}
